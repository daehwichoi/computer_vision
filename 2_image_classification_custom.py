...
def main():
    ...
    trainset = torchvision.datasets.CIFAR10('./data/train/',transform=train_transform,train=True,download=True)
    testset = torchvision.datasets.CIFAR10('./data/test/',transform=test_transform,train=False,download=True)

    train_loader = torch.utils.data.DataLoader(trainset,batch_size=batch_num, shuffle=True)
    test_loader = torch.utils.data.DataLoader(testset, batch_size=1000, shuffle=False)

    # torchvision model load 
    model = Resnet(num_classes=10).to(device=device)
